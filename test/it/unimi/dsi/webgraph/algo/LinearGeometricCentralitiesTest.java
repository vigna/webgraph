...
public class LinearGeometricCentralitiesTest {
	...
	public void testPath() throws InterruptedException {
		final ImmutableGraph graph = Transform.transpose(new ArrayListMutableGraph(3, new int[][] { { 0, 1 },
				{ 1, 2 } }).immutableView());
	...
	public void testErdosRenyi() throws IOException, InterruptedException {
		for (final int size : new int[] { 10, 100 }) {
			for (final double density : new double[] { 0.0001, 0.001, 0.01 }) {
				...
				for (int i = 0; i < size; i++) assertEquals(hanf.sumOfInverseDistances[i], linear.centrality[i], 1E-3);
				hanf.close();
			}
		}
	}
	...
	public void testErdosRenyiBis() throws IOException, InterruptedException {
		for (final int size : new int[] { 10, 100 }) {
			for (final double density : new double[] { 0.0001, 0.001, 0.01 }) {
				...
				for (int i = 0; i < size; i++) assertEquals(linear.reachable[i], linear.centrality[i], 1E-3);
				...
				for (int i = 0; i < size; i++) {
					if (centralities.closeness[i] != 0) assertEquals(-1.0 / centralities.closeness[i], linear.centrality[i], 1E-3);
